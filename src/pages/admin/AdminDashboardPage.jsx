...
function LineChart({ data, color = '#f0c050' }) {
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${w} ${h}`} style={{ overflow: 'visible' }}>
      ...
      {data.map((d, i) => (
        <text key={i} x={pts[i].x} y={h} textAnchor="middle" fontSize="9" fill="#888">{d.label}</text>
      ))}
    </svg>
  );
}
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.main}>
        ...
        {loading ? <p style={s.loading}>Loading dashboard...</p> : (
          <>
            {/* ── Stats Grid ── */}
            <div style={s.statsGrid}>
              {[
                { label: 'Total Buyers',     value: overview?.total_buyers,                          icon: '👥', color: '#1a4fa0' },
                { label: 'Total Sellers',    value: overview?.total_sellers,                         icon: '🏪', color: '#1f4d1f' },
                { label: 'Total Products',   value: overview?.total_products,                        icon: '🌾', color: '#c8860a' },
                { label: 'Total Orders',     value: overview?.total_orders,                          icon: '📦', color: '#7b2d8b' },
                { label: 'Pending Orders',   value: overview?.pending_orders,                        icon: '⏳', color: '#cc0000' },
                { label: 'Total Revenue',    value: `₦${Number(overview?.total_revenue||0).toLocaleString()}`, icon: '💵', color: '#1f4d1f' },
                { label: 'Active Loans',     value: loans?.active_loans,                             icon: '🔄', color: '#1a4fa0' },
                { label: 'Total Disbursed',  value: `₦${Number(loans?.total_disbursed||0).toLocaleString()}`,  icon: '💸', color: '#c8860a' },
                { label: 'Total Repaid',     value: `₦${Number(loans?.total_repaid||0).toLocaleString()}`,     icon: '✅', color: '#1f4d1f' },
              ].map((stat) => (
              ...
              <div style={s.chartsRow}>
                ...
                <div style={s.chartCard}>
                  <div style={s.chartHeader}>
                    <div>
                      ...
                      <div style={s.chartSub}>Last {chartData.length} months</div>
                    </div>
                ...
                <div style={s.chartCard}>
                  <div style={s.chartHeader}>
                    ...
                    <div style={{ ...s.chartBadge, background: '#fff8e7', color: '#b36b00' }}>Line</div>
                  </div>
              ...
              <div style={s.topRow}>
                  ...
                  {revenueStats.top_products?.map((p, i) => (
                    <div key={p.id} style={s.topItem}>
                      ...
                      <div style={s.topInfo}>
                        ...
                        <div style={s.topMeta}>{p.category} · {p.order_items_count} sold</div>
                      </div>
                      <div style={s.topValue}>₦{Number(p.price).toLocaleString()}</div>
                    </div>
                  ))}
                </div>
                ...
                <div style={s.topCard}>
                  <div style={s.activityHeader}>
                    ...
                    <span style={s.viewAll} onClick={() => navigate('/admin/sellers')}>View all</span>
                  </div>
                  {revenueStats.top_sellers?.map((sel, i) => (
                    <div key={sel.id} style={s.topItem}>
                      ...
                      <div style={s.topValue}>₦{Number(sel.earnings_balance).toLocaleString()}</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {loans && (
              <div style={s.loanSummaryRow}>
                {[
                  { label: 'Pending Applications', value: loans.pending_applications, color: '#b36b00', bg: '#fff8e7' },
                  { label: 'Active Loans',          value: loans.active_loans,         color: '#1a4fa0', bg: '#e7f0ff' },
                  { label: 'Total Disbursed',       value: `₦${Number(loans.total_disbursed).toLocaleString()}`, color: '#1f4d1f', bg: '#eafaf0' },
                  { label: 'Outstanding Balance',   value: `₦${Number(loans.outstanding).toLocaleString()}`,    color: '#cc0000', bg: '#fff0f0' },
                ].map(item => (
                  <div key={item.label} style={{ ...s.loanStatCard, background: item.bg }}>
                    <div style={{ ...s.loanStatValue, color: item.color }}>{item.value}</div>
            ...
            <div style={s.activityGrid}>
              <div style={s.activityCard}>
                <div style={s.activityHeader}>
                  ...
                  <span style={s.viewAll} onClick={() => navigate('/admin/orders')}>View all</span>
                </div>
                {recentOrders.length === 0 ? <p style={s.empty}>No orders yet</p> : recentOrders.map((order) => (
                  <div key={order.id} style={s.activityItem}>
                    <div>
                      <div style={s.activityId}>Order #{order.id}</div>
                      <div style={s.activityMeta}>{order.buyer?.name} — ₦{Number(order.total_amount || order.total || 0).toLocaleString()}</div>
                    </div>
              ...
              <div style={s.activityCard}>
                <div style={s.activityHeader}>
                  ...
                  <span style={s.viewAll} onClick={() => navigate('/admin/loans')}>View all</span>
                </div>
                {recentLoans.length === 0 ? <p style={s.empty}>No loans yet</p> : recentLoans.map((loan) => (
                  <div key={loan.id} style={s.activityItem}>
                    <div>
                      <div style={s.activityId}>₦{Number(loan.amount).toLocaleString()}</div>
                      <div style={s.activityMeta}>{loan.user?.name} — {loan.purpose}</div>
                    </div>
...
const s = {
  ...
  sidebarAvatar: { width: 34, height: 34, background: '#fff', borderRadius: '50%', overflow: 'hidden', border: '2px solid #f0c050' },
  ...
  chartBadge: { background: '#eafaf0', color: '#1f4d1f', fontSize: 10, fontWeight: 700, padding: '3px 8px', borderRadius: 4 },

  // Top products/sellers
  topRow: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 24 },
  topCard: { background: '#fff', borderRadius: 10, border: '1px solid #e8e4dc', padding: 20 },
  topItem: { display: 'flex', alignItems: 'center', gap: 12, padding: '10px 0', borderBottom: '1px solid #f5f5f5' },
  topRank: { fontSize: 12, fontWeight: 700, color: '#c8860a', width: 24 },
  topInfo: { flex: 1 },
  topName: { fontSize: 13, fontWeight: 600, color: '#111' },
  topMeta: { fontSize: 11, color: '#888', marginTop: 2, textTransform: 'capitalize' },
  topValue: { fontSize: 13, fontWeight: 700, color: '#1f4d1f' },

  // Loan summary
  loanSummaryRow: { display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 14, marginBottom: 24 },
  loanStatCard: { borderRadius: 10, padding: '16px 20px', border: '1px solid #e8e4dc' },
