...
export default function AdminReportsPage() {
  ...
  useEffect(() => {
    ...
    if (activeTab === 'loans') {
      ...
      api.get('/admin/reports/loans', { params: { days } })
  ...
  const MiniBarChart = ({ data, valueKey, labelKey, color = '#1f4d1f', height = 160 }) => {
    const safe = safeArr(data);
    if (safe.length === 0) return <p style={{ color: '#888', fontSize: 13, textAlign: 'center', padding: 20 }}>No data available</p>;
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'flex-end', gap: 6, height, padding: '0 4px' }}>
        {safe.map((item, i) => {
          ...
          const display = val >= 1000000 ? `₦${(val/1000000).toFixed(1)}M` : val >= 1000 ? `₦${(val/1000).toFixed(0)}K` : val > 0 ? `₦${val}` : '0';
          return (
            <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
              <div style={{ fontSize: 9, color: '#888', textAlign: 'center' }}>{display}</div>
              <div style={{ width: '100%', background: '#eee', borderRadius: 4, height: height - 40, display: 'flex', alignItems: 'flex-end' }}>
                <div style={{ width: '100%', height: `${Math.max(pct, 2)}%`, background: color, borderRadius: 4 }} />
              </div>
              <div style={{ fontSize: 9, color: '#888', textAlign: 'center' }}>
                {item[labelKey] === 1 ? 'Jan' : item[labelKey] === 2 ? 'Feb' : item[labelKey] === 3 ? 'Mar' : item[labelKey] === 4 ? 'Apr' : item[labelKey] === 5 ? 'May' : item[labelKey] === 6 ? 'Jun' : item[labelKey] === 7 ? 'Jul' : item[labelKey] === 8 ? 'Aug' : item[labelKey] === 9 ? 'Sep' : item[labelKey] === 10 ? 'Oct' : item[labelKey] === 11 ? 'Nov' : item[labelKey] === 12 ? 'Dec' : item[labelKey]}
              </div>
            </div>
          );
        })}
  ...
  const MiniPieChart = ({ data, labelKey, valueKey }) => {
    const safe = safeArr(data);
    if (safe.length === 0) return <p style={{ color: '#888', fontSize: 13, textAlign: 'center', padding: 20 }}>No data</p>;
    const colors = ['#1f4d1f', '#f0c050', '#1a4fa0', '#cc0000', '#1a7a3a', '#b36b00', '#0077aa'];
    const total = safe.reduce((s, d) => s + Number(d[valueKey] || 0), 0);
    return (
      <div>
        {safe.map((item, i) => {
          const pct = total > 0 ? Math.round((Number(item[valueKey] || 0) / total) * 100) : 0;
          return (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
              <div style={{ width: 12, height: 12, borderRadius: 3, background: colors[i % colors.length], flexShrink: 0 }} />
              <div style={{ flex: 1, fontSize: 13, color: '#333', textTransform: 'capitalize' }}>{item[labelKey]}</div>
              <div style={{ width: 120, background: '#eee', borderRadius: 99, overflow: 'hidden', height: 8 }}>
                <div style={{ width: `${pct}%`, height: '100%', background: colors[i % colors.length] }} />
              </div>
              <div style={{ fontSize: 12, fontWeight: 600, color: '#555', minWidth: 36, textAlign: 'right' }}>{pct}%</div>
              <div style={{ fontSize: 12, color: '#888', minWidth: 30, textAlign: 'right' }}>{item[valueKey]}</div>
            </div>
          );
        })}
      </div>
  ...
  const downloadSalesPDF = () => {
    ...
    const topRows = safeArr(salesData.top_products).map((p, i) => `
    ...
    if (!win) { showToast('Allow popups to download PDF.'); setDownloading(false); return; }
  ...
  const downloadLoansPDF = () => {
    ...
    setDownloading(true);
    const today = new Date().toLocaleDateString('en-NG', { year: 'numeric', month: 'long', day: 'numeric' });
    const staffRows = safeArr(loansData.staff_performance).map((s, i) => `
      <tr style="background:${i % 2 === 0 ? '#f9fafb' : '#fff'}">
  ...
  const downloadSalesCSV = () => {
    ...
    const rows = [
      ...
      ...safeArr(salesData.top_products).map((p, i) => [i + 1, p.name, p.category, p.items_sold || 0, p.revenue || 0]),
    ];
    downloadCSV(rows, `achoice-sales-${new Date().toISOString().slice(0, 10)}.csv`);
  };
  ...
  const downloadLoansCSV = () => {
    ...
    const rows = [
      ...
      ...safeArr(loansData.staff_performance).map(s => [s.name, s.total_reviewed || 0, s.approved || 0, s.rejected || 0, s.total_disbursed || 0]),
    ];
    downloadCSV(rows, `achoice-loans-${new Date().toISOString().slice(0, 10)}.csv`);
  };
      ...
      <div style={s.main}>
        <div style={s.header}>
          <div>
            ...
            <p style={s.headerSub}>Download and analyze platform performance data</p>
          </div>
          <div style={s.periodRow}>
            {[7, 30, 90, 365].map(d => (
              <button key={d} style={days === d ? s.periodBtnActive : s.periodBtn} onClick={() => setDays(d)}>
        ...
        {/* Tabs */}
        <div style={s.tabNav}>
          {[{ key: 'sales', label: '📦 Sales Report' }, { key: 'loans', label: '💰 Loans Report' }, { key: 'activity', label: '👥 Staff Activity' }].map(tab => (
        ...
        {activeTab === 'sales' && (
          <div>
            <div style={s.downloadRow}>
              <button style={downloading ? s.dlBtnDisabled : { ...s.dlBtn, background: '#cc0000' }} onClick={downloadSalesPDF} disabled={downloading}>📄 PDF</button>
              <button style={downloading ? s.dlBtnDisabled : { ...s.dlBtn, background: '#1a7a3a' }} onClick={downloadSalesCSV} disabled={downloading}>📋 CSV</button>
            </div>

            {salesLoading ? <p style={s.loading}>Loading sales report...</p> : salesData ? (
              <>
                <div style={s.kpiGrid}>
                  {[
                    { label: 'Total Revenue', value: toMoney(salesData.total_revenue), icon: '💵', color: '#1f4d1f' },
                    { label: 'Total Orders', value: salesData.total_orders || 0, icon: '📦', color: '#1a4fa0' },
                    { label: 'Delivered', value: salesData.delivered_orders || 0, icon: '✅', color: '#1a7a3a' },
                    { label: 'Pending', value: salesData.pending_orders || 0, icon: '⏳', color: '#b36b00' },
                    { label: 'Avg Order Value', value: toMoney(salesData.average_order_value), icon: '📊', color: '#1f4d1f' },
                    { label: 'Delivery Fees', value: toMoney(salesData.delivery_fees_collected), icon: '🚚', color: '#555' },
                  ].map(kpi => (
                    <div key={kpi.label} style={s.kpiCard}>
                      <div style={s.kpiTop}><div style={s.kpiLabel}>{kpi.label}</div><span style={s.kpiIcon}>{kpi.icon}</span></div>
                ...
                <div style={s.chartsRow}>
                  <div style={s.chartCard}>
                    ...
                    <MiniBarChart data={salesData.monthly_revenue} valueKey="revenue" labelKey="month" color="#1f4d1f" />
                  </div>
                  <div style={s.chartCard}>
                    <div style={s.chartTitle}>Orders by Status</div>
                    <MiniPieChart data={salesData.orders_by_status} labelKey="status" valueKey="count" />
                  </div>
                </div>
                      ...
                      <tbody>
                        {safeArr(salesData.top_products).map((p, i) => (
                          <tr key={i} style={s.tableRow}>
                            <td style={s.td}><strong style={{ color: '#f0c050' }}>#{i + 1}</strong></td>
                            <td style={{ ...s.td, fontWeight: 600 }}>{p.name || '—'}</td>
                            <td style={s.td}><span style={s.categoryBadge}>{p.category || '—'}</span></td>
                            <td style={s.td}>{p.items_sold || 0}</td>
                            <td style={{ ...s.td, color: '#1f4d1f', fontWeight: 700 }}>{toMoney(p.revenue)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                    {safeArr(salesData.top_products).length === 0 && <div style={s.empty}>No product data for this period.</div>}
                  </div>
                </div>
              </>
        ...
        {activeTab === 'loans' && (
          <div>
            <div style={s.downloadRow}>
              <button style={downloading ? s.dlBtnDisabled : { ...s.dlBtn, background: '#cc0000' }} onClick={downloadLoansPDF} disabled={downloading}>📄 PDF</button>
              <button style={downloading ? s.dlBtnDisabled : { ...s.dlBtn, background: '#1a7a3a' }} onClick={downloadLoansCSV} disabled={downloading}>📋 CSV</button>
            </div>

            {loansLoading ? <p style={s.loading}>Loading loans report...</p> : loansData ? (
              <>
                <div style={s.kpiGrid}>
                  {[
                    { label: 'Total Applications', value: loansData.total_applications || 0, icon: '📋', color: '#1f4d1f' },
                    { label: 'Approved', value: loansData.approved || 0, icon: '✅', color: '#1a7a3a' },
                    { label: 'Rejected', value: loansData.rejected || 0, icon: '❌', color: '#cc0000' },
                    { label: 'Pending', value: loansData.pending || 0, icon: '⏳', color: '#b36b00' },
                    { label: 'Total Disbursed', value: toMoney(loansData.total_disbursed), icon: '💸', color: '#1f4d1f' },
                    { label: 'Total Repaid', value: toMoney(loansData.total_repaid), icon: '💰', color: '#1a7a3a' },
                    { label: 'Outstanding', value: toMoney(loansData.outstanding_balance), icon: '⚠️', color: '#cc0000' },
                    { label: 'Repayment Rate', value: loansData.repayment_rate || '0%', icon: '📊', color: '#1a4fa0' },
                  ].map(kpi => (
                    <div key={kpi.label} style={s.kpiCard}>
                      <div style={s.kpiTop}><div style={s.kpiLabel}>{kpi.label}</div><span style={s.kpiIcon}>{kpi.icon}</span></div>
                      <div style={{ ...s.kpiValue, color: kpi.color }}>{kpi.value}</div>
                ...
                <div style={s.chartsRow}>
                  <div style={s.chartCard}>
                    ...
                    <MiniBarChart data={salesData?.monthly_revenue ? loansData.monthly_loans : loansData.monthly_loans} valueKey="amount" labelKey="month" color="#1a4fa0" />
                  </div>
                  <div style={s.chartCard}>
                    <div style={s.chartTitle}>Loans by Purpose</div>
                    <MiniPieChart data={loansData.by_purpose} labelKey="purpose" valueKey="count" />
                  </div>
                </div>

                {safeArr(loansData.staff_performance).length > 0 && (
                  <div style={s.tableCard}>
                    <div style={s.tableCardTitle}>👥 Loan Staff Performance</div>
                    <div style={{ overflowX: 'auto' }}>
                      <table style={s.table}>
                        <thead><tr style={s.tableHead}>
                        ...
                        <tbody>
                          {safeArr(loansData.staff_performance).map((staff, i) => (
                            <tr key={i} style={s.tableRow}>
                              <td style={{ ...s.td, fontWeight: 600 }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                  <div style={{ width: 32, height: 32, background: '#1f4d1f', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#f0c050', fontWeight: 700, fontSize: 13, flexShrink: 0 }}>
                                    {staff.name?.charAt(0) || 'S'}
                                  </div>
                                  {staff.name || '—'}
                                </div>
                              </td>
                              <td style={s.td}>{staff.total_reviewed || 0}</td>
                              <td style={{ ...s.td, color: '#1a7a3a', fontWeight: 700 }}>{staff.approved || 0}</td>
                              <td style={{ ...s.td, color: '#cc0000', fontWeight: 700 }}>{staff.rejected || 0}</td>
                              <td style={{ ...s.td, color: '#1f4d1f', fontWeight: 700 }}>{toMoney(staff.total_disbursed)}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
          ...
          <div>
            <div style={s.activityControls}>
              <div style={s.filterTabs}>
                {[{ key: 'all', label: 'All Activity' }, { key: 'loan', label: '💰 Loan' }, { key: 'agro', label: '🌾 Agro/Sales' }].map(f => (
                  <button key={f.key} style={activityFilter === f.key ? s.filterTabActive : s.filterTab} onClick={() => setActivityFilter(f.key)}>
                    {f.label}
                  </button>
                ))}
              </div>
            ...
            {activityLoading ? <p style={s.loading}>Loading activity...</p> : activityData ? (
              <>
                {activityData.summary && (
                  <div style={s.activitySummary}>
                    {[
                      { label: 'Total Actions', value: activityData.summary.total_actions || 0 },
                      { label: 'Agro Actions', value: activityData.summary.agro_actions || 0 },
                      { label: 'Loan Actions', value: activityData.summary.loan_actions || 0 },
                      { label: 'Active Staff', value: activityData.summary.active_staff || 0 },
                    ].map(item => (
                ...
                <div style={s.tableCard}>
                  <div style={s.tableCardTitle}>
                    Staff Activity Log
                    {activityData.total > 0 && <span style={{ fontSize: 12, color: '#888', fontWeight: 400, marginLeft: 8 }}>({activityData.total} total actions)</span>}
                  </div>
                  <div style={{ overflowX: 'auto' }}>
                    <table style={s.table}>
                      <thead><tr style={s.tableHead}>
                          ...
                          <tr key={act.id || i} style={s.tableRow}>
                            <td style={s.td}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                ...
                                <div>
                                  <div style={{ fontWeight: 600, color: '#111', fontSize: 13 }}>{act.staff?.name || '—'}</div>
                                  <div style={{ fontSize: 11, color: '#888' }}>{act.staff?.email || ''}</div>
                                </div>
                              </div>
                            </td>
                            <td style={s.td}>
                              <span style={{ background: '#f7f5f0', color: '#555', padding: '3px 8px', borderRadius: 5, fontSize: 11, fontWeight: 600, textTransform: 'replace', whiteSpace: 'nowrap' }}>
                            ...
                            <td style={s.td}>
                              <span style={{
                                ...
                                background: act.module === 'loan' ? '#e7f0ff' : act.module === 'agro' ? '#eafaf0' : '#f0f0f0',
                                color: act.module === 'loan' ? '#1a4fa0' : act.module === 'agro' ? '#1a7a3a' : '#555',
                              }}>
                                {act.module || '—'}
                              </span>
                            </td>
                            <td style={{ ...s.td, fontSize: 12, color: '#555', maxWidth: 260 }}>{act.details || '—'}</td>
                            <td style={{ ...s.td, fontSize: 11, color: '#888', whiteSpace: 'nowrap' }}>{fmtDate(act.created_at)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
...
const s = {
  ...
  toast: { position: 'fixed', top: 20, right: 20, background: '#1f4d1f', color: '#fff', padding: '12px 24px', borderRadius: 8, fontSize: 14, zIndex: 9999 },
  ...
  periodBtn: { padding: '8px 16px', border: '1px solid #ddd', borderRadius: 6, background: '#fff', color: '#555', fontSize: 13, cursor: 'pointer', fontFamily: 'inherit' },
  periodBtnActive: { padding: '8px 16px', border: '1px solid #1f4d1f', borderRadius: 6, background: '#1f4d1f', color: '#fff', fontSize: 13, cursor: 'pointer', fontFamily: 'inherit' },
  tabNav: { display: 'flex', gap: 4, marginBottom: 24, background: '#fff', padding: 6, borderRadius: 10, border: '1px solid #e8e4dc', flexWrap: 'wrap' },
  tab: { flex: 1, padding: '10px 16px', border: 'none', borderRadius: 7, background: 'transparent', color: '#555', fontSize: 14, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit' },
  tabActive: { flex: 1, padding: '10px 16px', border: 'none', borderRadius: 7, background: '#1f4d1f', color: '#fff', fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' },
  downloadRow: { display: 'flex', gap: 10, marginBottom: 20, flexWrap: 'wrap' },
  dlBtn: { padding: '10px 20px', color: '#fff', border: 'none', borderRadius: 7, fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' },
  dlBtnDisabled: { padding: '10px 20px', background: '#ccc', color: '#fff', border: 'none', borderRadius: 7, fontSize: 13, cursor: 'not-allowed', fontFamily: 'inherit' },
  kpiGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 14, marginBottom: 20 },
  kpiCard: { background: '#fff', borderRadius: 10, border: '1px solid #e8e4dc', padding: 18 },
  ...
  chartCard: { background: '#fff', borderRadius: 10, border: '1px solid #e8e4dc', padding: 20 },
  chartTitle: { fontSize: 14, fontWeight: 700, color: '#111', marginBottom: 16 },
  tableCard: { background: '#fff', borderRadius: 10, border: '1px solid #e8e4dc', overflow: 'hidden', marginBottom: 20 },
  tableCardTitle: { fontSize: 14, fontWeight: 700, color: '#111', padding: '16px 20px', borderBottom: '1px solid #eee' },
  table: { width: '100%', borderCollapse: 'collapse', minWidth: 500 },
  tableHead: { background: '#f7f5f0', borderBottom: '2px solid #eee' },
  th: { padding: '12px 16px', textAlign: 'left', fontSize: 11, color: '#666', textTransform: 'uppercase', letterSpacing: 0.5, whiteSpace: 'nowrap' },
  tableRow: { borderTop: '1px solid #f5f5f5' },
  td: { padding: '14px 16px', verticalAlign: 'middle', fontSize: 13 },
  categoryBadge: { background: '#eef2ff', color: '#4338ca', padding: '3px 8px', borderRadius: 6, fontSize: 11, fontWeight: 600 },
  ...
  activitySummary: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: 14, marginBottom: 20 },
  summaryCard: { background: '#fff', borderRadius: 10, border: '1px solid #e8e4dc', padding: 18, textAlign: 'center' },
  summaryVal: { fontSize: 28, fontWeight: 700, color: '#1f4d1f', marginBottom: 4 },
