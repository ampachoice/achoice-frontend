...
export default function DeliveryZonesPage() {
  ...
  const handleSaveAll = async () => {
    ...
    for (const id of editedIds) {
      ...
      try {
        ...
        setZones((prev) =>
          prev.map((z) => (z.id === Number(id) ? { ...z, ...payload } : z)),
        );
    ...
    setEditedZones({});
    showToast(
      `${successCount} zone${successCount !== 1 ? "s" : ""} updated successfully!`,
    );
  };
  ...
  const stats = {
    ...
    mostExpensive: zones.length
      ? Math.max(...zones.map((z) => Number(z.fee)))
      : 0,
  };
        ...
        <div style={s.statsGrid}>
          ...
          <div style={s.statCard}>
            ...
            <div style={{ ...s.statValue, color: "#cc0000" }}>
              ₦{stats.mostExpensive.toLocaleString()}
            </div>
          </div>
          <div style={s.statCard}>
            <div style={s.statLabel}>Free Delivery Above</div>
            <div style={{ ...s.statValue, color: "#1f4d1f" }}>
              ₦{Number(freeThreshold).toLocaleString()}
            </div>
          </div>
        </div>
        ...
        <div style={s.thresholdCard}>
          <div style={s.thresholdLeft}>
            ...
            <div style={s.thresholdSub}>
              Orders above this amount get free delivery. Currently: ₦
              {Number(freeThreshold).toLocaleString()}
            </div>
          </div>
