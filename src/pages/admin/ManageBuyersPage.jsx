...
export default function ManageBuyersPage() {
  ...
  const handleActivate = async (buyer) => {
    if (
      !window.confirm(
        `Lift all restrictions on ${buyer.name} and restore active status?`,
      )
    )
      return;
    setActing(buyer.id);
  ...
  const handleRestrictSubmit = async (e) => {
    ...
    try {
      const res = await api.patch(
        `/admin/users/${restrictModal.id}/restrict`,
        restrictForm,
      );
      const updated = res.data?.user || res.data;
      setBuyers((prev) =>
        prev.map((b) => (b.id === restrictModal.id ? { ...b, ...updated } : b)),
      );
      showToast(res.data?.message || "Restrictions updated.");
      setRestrictModal(null);
    } catch (err) {
      showToast(
        err.response?.data?.message || "Failed to update restrictions.",
      );
    } finally {
                ...
                <label style={s.checkRow}>
                  <input
                    ...
                    onChange={(e) =>
                      setRestrictForm({
                        ...restrictForm,
                        restrict_orders: e.target.checked,
                      })
                    }
                  />
                ...
                <label style={s.checkRow}>
                  <input
                    ...
                    onChange={(e) =>
                      setRestrictForm({
                        ...restrictForm,
                        restrict_loans: e.target.checked,
                      })
                    }
                  />
          ...
          <div key={buyer.id} style={s.buyerCard}>
            <div style={s.buyerHeader}>
              <div>
                ...
                <div style={s.buyerMeta}>
                  Joined {fmtDate(buyer.created_at)}
                </div>
              </div>
              <div style={s.buyerHeaderRight}>
                <div
                  style={{ ...s.statusBadge, ...getStatusStyle(buyer.status) }}
                >
                  {buyer.status}
                </div>
                {buyer.open_complaints > 0 && (
                  <div
                    style={s.complaintBadge}
                    onClick={() =>
                      navigate("/admin/complaints?user=" + buyer.id)
                    }
                  >
                    📋{buyer.open_complaints}
                  </div>
                )}
                {!!buyer.restrict_orders && (
                  <div style={s.restrictBadge}>🚫 Orders restricted</div>
                )}
                {!!buyer.restrict_loans && (
                  <div style={s.restrictBadge}>🚫 Loans restricted</div>
                )}
              </div>
            </div>
            ...
            <div style={s.actionRow}>
              ...
              <button
                style={
                  acting === buyer.id ? s.restrictBtnDisabled : s.restrictBtn
                }
              ...
              {(buyer.status !== "active" ||
                ...
                <button
                  style={
                    acting === buyer.id ? s.activateBtnDisabled : s.activateBtn
                  }
              ...
              <div style={s.detailPanel}>
                ...
                {detailMap[buyer.id] && !detailMap[buyer.id].error && (
                  <>
                    ...
                    <div style={{ ...s.detailSectionTitle, marginTop: 16 }}>
                      Recent Loans
                    </div>
