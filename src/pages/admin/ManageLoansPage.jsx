...
export default function ManageLoansPage() {
  ...
  const fetchDocsForLoan = async (loanId) => {
    try {
      ...
      setDocsMap(prev => ({ ...prev, [loanId]: Array.isArray(raw) ? raw : [] }));
    } catch (err) {
      const status = err.response?.status;
      const msg = err.response?.data?.message || err.message;
      if (status === 403) {
        setDocsMap(prev => ({ ...prev, [loanId]: `403: ${msg || 'Forbidden'}` }));
      } else {
  ...
  const handleApproveSubmit = async (e) => {
    ...
    try {
      const res = await api.patch(`/admin/loans/${loan.id}/decision`, {
        decision: 'approved',
        interest_rate: Number(loanSettings?.default_interest || approveForm.interest_rate),
  ...
  const toMoney = (val) => val != null && !isNaN(Number(val)) ? `₦${Number(val).toLocaleString()}` : '—';

  const docTypeIcon = (t) => ({ nin_document: '🪪', bvn_document: '🏦', statement_of_account: '📊', collateral_document: '🏠', collateral_image: '📸', other: '📄' }[t] || '📄');
  const docTypeLabel = (t) => ({ nin_document: 'NIN Document', bvn_document: 'BVN Document', statement_of_account: 'Bank Statement', collateral_document: 'Collateral', collateral_image: 'Collateral Image', other: 'Other' }[t] || (t || 'Document').replace(/_/g, ' '));
    ...
    <div style={s.page}>
            ...
            <div style={s.modalBody}>
              ...
              <form onSubmit={handleApproveSubmit}>
                <div style={s.modalField}>
                  <label style={s.modalLabel}>Interest Rate (%) <span style={s.modalLabelNote}>— from loan settings</span></label>
                  <div style={s.modalRateDisplay}>
                    {loanSettings?.default_interest || approveForm.interest_rate}% flat
                  </div>
                </div>
                <div style={s.modalField}>
                  <label style={s.modalLabel}>Loan Duration <span style={s.modalLabelNote}>— select from approved options</span></label>
                  {loanSettings?.durations?.length > 0 ? (
                    <div style={s.durationOptions}>
                      {loanSettings.durations.map(d => (
                        <button key={d.id} type="button"
                          style={{ ...s.durationBtn, ...(Number(approveForm.duration_months) === d.months ? s.durationBtnActive : {}) }}
                    ...
                    <input style={s.modalInput} type="number" value={approveForm.duration_months}
                ...
                <div style={s.modalFooter}>
                  <button type="button" style={s.modalCancelBtn} onClick={() => setShowApproveModal(null)}>Cancel</button>
                  <button type="submit" style={s.modalApproveBtn}>✓ Confirm Approval</button>
                </div>
              </form>
            </div>
          </div>
      ...
      <div style={s.sidebar}>
        ...
        <div style={s.sidebarFooter}>
          <button style={s.logoutBtn} onClick={() => {
            localStorage.removeItem('token'); localStorage.removeItem('user'); navigate('/admin');
          }}>Logout</button>
        </div>
      </div>
      ...
      <div style={s.main}>
        ...
        <div style={s.filterRow}>
          <div style={s.filterTabs}>
            {['all','pending','approved','disbursed','active','rejected','completed'].map(tab => (
              <button key={tab} style={filter===tab ? s.filterTabActive : s.filterTab}
          ...
          <input style={s.searchInput} type="text" placeholder="Search by name, email or purpose..."
        ...
        {filtered.map(loan => {
          ...
          const appDocs = Array.isArray(docsMap[loan.id]) ? docsMap[loan.id] : [];
          const docsError = typeof docsMap[loan.id] === 'string' ? docsMap[loan.id] : null;
            ...
            <div key={loan.id} style={s.loanCard}>
              <div style={s.loanHeader}>
                <div>
                  ...
                  <div style={s.loanApplicant}>{loan.user?.name || 'Applicant'} — {loan.user?.email || ''}</div>
                  {loan.user?.phone && <div style={{ fontSize:12, color:'#aaa' }}>📞 {loan.user.phone}</div>}
                </div>
                <div style={s.loanHeaderRight}>
                  ...
                  {successfulRepayments.length > 0 && (
                    <div style={s.repayCountBadge}>{successfulRepayments.length} payment{successfulRepayments.length > 1 ? 's' : ''} confirmed</div>
                  )}
                </div>
              </div>
              ...
              {(loan.nin_number || loan.bvn_number) && (
                <div style={{ display:'flex', gap:10, marginBottom:14, flexWrap:'wrap' }}>
                  {loan.nin_number && <span style={s.idBadge}>🪪 NIN: {loan.nin_number}</span>}
                  {loan.bvn_number && <span style={s.idBadge}>🏦 BVN: {loan.bvn_number}</span>}
                </div>
              )}
              ...
              {loan.status === 'pending' && (
                <div style={s.actionRow}>
                  <div style={s.actionNote}>
                    ...
                    {appDocs.length > 0 && <span style={{ marginLeft:8, color:'#1f4d1f', fontWeight:600 }}>📎 {appDocs.length} doc(s) uploaded, {verifiedCount} verified</span>}
                    {appDocs.length === 0 && docsMap[loan.id] !== undefined && !docsError && <span style={{ marginLeft:8, color:'#cc0000' }}>⚠️ No documents uploaded</span>}
                  </div>
                  <div style={s.actionBtns}>
                    <button style={deciding===loan.id ? s.approveBtnDisabled : s.approveBtn}
                      onClick={() => handleDecision(loan.id, 'approved')} disabled={deciding===loan.id}>
                      {deciding===loan.id ? 'Processing...' : '✓ Approve Loan'}
                    </button>
                    <button style={deciding===loan.id ? s.rejectBtnDisabled : s.rejectBtn}
                      onClick={() => handleDecision(loan.id, 'rejected')} disabled={deciding===loan.id}>
                      ✕ Reject
                    </button>
                  </div>
                </div>
              ...
              {loan.status === 'approved' && (
                <div style={s.disburseSection}>
                  ...
                  <div style={s.disburseInstructions}>
                    <div style={s.disburseStep}><div style={s.disburseStepNum}>1</div><div>Transfer <strong>{toMoney(loan.amount)}</strong> to the borrower's bank account manually</div></div>
                    <div style={s.disburseStep}><div style={s.disburseStepNum}>2</div><div>After confirming the transfer, click below to activate the repayment schedule</div></div>
                  </div>
                  {loan.user && (
                  ...
                  <button style={disbursing===loan.id ? s.disburseBtnDisabled : s.disburseBtn}
                    onClick={() => handleDisburse(loan)} disabled={disbursing===loan.id}>
                  ...
                  <div style={s.activeSectionRow}>
                    <div>
                      <div style={s.activeInfo}>
                        {loan.status==='disbursed' ? '✅ Disbursed — repayment schedule activated.' : '🔄 Active — buyer is making repayments.'}
                      </div>
                  ...
                  {expandedLoan===loan.id && successfulRepayments.length>0 && (
                    <div style={s.repaymentsTable}>
                      <div style={s.repaymentsHead}>
                        <span>#</span><span>Paid On</span><span>Amount</span><span>Reference</span><span>Balance After</span><span>Status</span>
                      </div>
                      {successfulRepayments.map((r,i) => (
                        <div key={r.id||i} style={{ ...s.repaymentsRow, background:'#f0fff4' }}>
                          ...
                          <span style={{ fontSize:10, color:'#888', wordBreak:'break-all' }}>{r.payment_reference?r.payment_reference.slice(-12):'—'}</span>
                          <span>{toMoney(r.balance_after)}</span>
                          <span><span style={{ ...s.statusBadge, ...getStatusStyle(r.status), fontSize:10, padding:'2px 8px' }}>{r.status}</span></span>
                        </div>
                      ))}
                    </div>
                  )}
              ...
              {loan.status==='rejected' && <div style={s.rejectedNote}>✕ Rejected{loan.rejection_reason?`: ${loan.rejection_reason}`:''}</div>}
              {loan.status==='completed' && <div style={s.completedNote}>🎉 Fully repaid and completed on {fmtDate(loan.completed_at)}.</div>}
              ...
              <div style={s.docsSection}>
                <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:8 }}>
                  <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                    ...
                    {appDocs.length > 0 && (
                      <span style={s.docCountBadge}>{appDocs.length} uploaded · {verifiedCount} verified</span>
                    )}
                    {docsError && <span style={{ fontSize:11, color:'#cc0000', fontWeight:600 }}>⛔ 403 — Contact Sherif</span>}
                    {!docsError && appDocs.length === 0 && docsMap[loan.id] !== undefined && (
                      <span style={{ fontSize:11, color:'#888' }}>No documents uploaded</span>
                    )}
                    {docsMap[loan.id] === undefined && <span style={{ fontSize:11, color:'#aaa' }}>Loading...</span>}
                  </div>
                  <button style={s.docsToggleBtn} onClick={() => handleViewDocs(loan.id)}>
                ...
                {expandedDocs === loan.id && (
                  <div style={s.docsPanel}>
                    ...
                    {docsLoading && docsMap[loan.id] === undefined ? (
                      <p style={{ textAlign:'center', color:'#888', padding:20 }}>Loading documents...</p>
                    ) : docsError ? (
                      <div style={{ background:'#fff0f0', border:'1px solid #ffb3b3', borderRadius:8, padding:16, marginBottom:16 }}>
                        <div style={{ fontSize:13, fontWeight:700, color:'#cc0000', marginBottom:6 }}>⛔ Cannot Load Documents</div>
                        <div style={{ fontSize:12, color:'#cc0000' }}>{docsError}</div>
                        <button style={{ marginTop:10, padding:'7px 14px', background:'#1f4d1f', color:'#fff', border:'none', borderRadius:6, fontSize:12, cursor:'pointer', fontFamily:'inherit' }}
                          onClick={() => fetchDocsForLoan(loan.id)}>🔄 Retry</button>
                      </div>
                    ) : appDocs.length === 0 ? (
                      <div style={s.noDocsBox}>
                        <div style={{ fontSize:32, marginBottom:8 }}>📄</div>
                        <p style={{ fontSize:13, color:'#888' }}>No documents uploaded yet for this application.</p>
                      </div>
                    ) : (
                          ...
                          <div key={doc.id} style={s.docCard}>
                            ...
                            <div style={s.docCardBody}>
                              ...
                              {doc.is_verified
                                ...
                                : <div style={s.pendingBadge}>⏳ Pending review</div>
                              }
                            </div>
                            <div style={s.docCardActions}>
                              ...
                              {!doc.is_verified && (
                                <button style={verifyingDoc===doc.id ? s.verifyDocBtnDisabled : s.verifyDocBtn}
                                  onClick={() => handleVerifyDoc(doc.id, loan.id)} disabled={verifyingDoc===doc.id}>
                                  {verifyingDoc===doc.id ? '⏳' : '✓ Verify'}
                                </button>
                              )}
                              <button style={s.delDocBtn} onClick={() => handleDeleteDoc(doc.id, loan.id)}>🗑</button>
                            </div>
                          </div>
                        ))}
                      </div>
                    ...
                    <div style={s.uploadSection}>
                      ...
                      <div style={s.uploadRow}>
                        ...
                        <button style={uploadingDoc ? s.uploadBtnDisabled : s.uploadBtn}
                          type="button" onClick={() => handleUploadDoc(loan.id)} disabled={uploadingDoc}>
                          {uploadingDoc ? '⏳ Uploading...' : '⬆ Upload'}
                        </button>
                      </div>
                      {docForm.file && (
                        <div style={{ fontSize:12, color:'#1a7a3a', marginTop:6 }}>
...
const s = {
  ...
  loanHeader: { display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:16, flexWrap:'wrap', gap:10 },
  ...
  idBadge: { fontSize:12, background:'#e7f0ff', color:'#1a4fa0', padding:'4px 10px', borderRadius:6, fontWeight:600 },
  actionRow: { display:'flex', flexDirection:'column', gap:10, marginBottom:14 },
  ...
  disburseSection: { background:'#f0f7ec', borderRadius:8, padding:20, border:'1px solid #c5ddb8', marginBottom:14 },
  ...
  activeSection: { background:'#eafaf0', borderRadius:8, padding:16, border:'1px solid #c5ddb8', marginBottom:14 },
  ...
  docCountBadge: { background:'#1f4d1f', color:'#fff', fontSize:10, fontWeight:700, padding:'2px 10px', borderRadius:99 },
  ...
  verifiedBadge: { fontSize:10, fontWeight:600, color:'#1a7a3a', background:'#eafaf0', padding:'2px 6px', borderRadius:4, display:'inline-block' },
  pendingBadge: { fontSize:10, fontWeight:600, color:'#b36b00', background:'#fff8e7', padding:'2px 6px', borderRadius:4, display:'inline-block' },
  ...
  verifyDocBtnDisabled: { padding:'5px 10px', background:'#f0f0f0', color:'#aaa', border:'1px solid #ddd', borderRadius:5, fontSize:11, cursor:'not-allowed', fontFamily:'inherit' },
