    ...
    try {
      ...
      showToast(
        err.response?.data?.message ||
          "Verification failed. Check the reference and try again.",
      );
    } finally {
      setVerifying(null);
    }
      ...
      >
        ...
        {filtered.map((order) => {
          ...
          return (
            <div
              key={order.id}
              style={{ ...s.orderCard, ...(isUnpaid ? s.orderCardUnpaid : {}) }}
            >
                ...
                <div style={s.orderHeaderRight}>
                  ...
                  <div
                    style={{
                      ...s.paymentBadge,
                      background:
                        order.payment_status === "paid" ? "#eafaf0" : "#fff0f0",
                      color:
                        order.payment_status === "paid" ? "#1a7a3a" : "#cc0000",
                    }}
                  >
              ...
              {isUnpaid && (
                <div style={s.unpaidBanner}>
                  <div style={s.unpaidBannerText}>
                    Payment not verified — buyer may have paid via Paystack but
                    webhook failed
                  </div>
                  <button
                    style={
                      verifying === order.id ? s.verifyBtnDisabled : s.verifyBtn
                    }
                    onClick={() =>
              ...
              <div style={s.orderFooter}>
                <div style={s.deliveryInfo}>
                  ...
                  {order.delivery_fee > 0 && (
                    <span style={s.deliveryFee}>
                      {" "}
                      · Fee: {toMoney(order.delivery_fee)}
                    </span>
                  )}
                </div>
              ...
              <div style={s.actionRow}>
                ...
                {(order.status === "pending" ||
                  ...
                  <button
                    style={
                      updating === order.id ? s.cancelBtnDisabled : s.cancelBtn
                    }
