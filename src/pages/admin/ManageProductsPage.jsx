...
export default function ManageProductsPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    price: '', discount_price: '', quantity: '', unit: 'bag',
    category: '', status: 'available', min_order_qty: 1,
  });
  ...
  const handleEditOpen = (product) => {
    ...
    setEditFile(null);
    setEditPreview(product.images?.[0]?.image_url || product.images?.[0]?.url || product.image || null);
  };
  ...
  const handleEditSubmit = async (e) => {
    ...
    if (editFile) {
      ...
      try {
        const cloudRes = await axios.post('https://api.cloudinary.com/v1_1/ds4wspou1/image/upload', cloudData);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setShowForm(false);
      setFormData({ seller_id: '', name: '', description: '', price: '', discount_price: '', quantity: '', unit: 'bag', category: '', status: 'available', min_order_qty: 1 });
    ...
    <div style={s.page}>
                ...
                <div style={s.formGrid}>
                  <div style={s.field}>
                    ...
                    <select style={s.input} name="seller_id" value={editForm.seller_id} onChange={handleEditChange} required>
                      ...
                      {sellers.map(sel => <option key={sel.id} value={sel.id}>{sel.business_name || sel.name}</option>)}
                    </select>
                  </div>
                  <div style={s.field}>
                  ...
                  <div style={s.field}>
                    ...
                    <select style={s.input} name="category" value={editForm.category} onChange={handleEditChange} required>
                      ...
                      {categories.map(cat => <option key={cat.slug || cat.id} value={cat.slug}>{cat.name}</option>)}
                    </select>
                  </div>
                  <div style={s.field}>
                  ...
                  <div style={s.field}>
                    ...
                    <input style={s.input} type="number" name="discount_price" value={editForm.discount_price} onChange={handleEditChange} placeholder="Optional" />
                  </div>
                  <div style={s.field}>
                  ...
                  <div style={s.field}>
                    ...
                    <select style={s.input} name="unit" value={editForm.unit} onChange={handleEditChange}>
                      {['bag', 'kg', 'ton', 'crate', 'litre', 'piece', 'bunch'].map(u => <option key={u} value={u}>{u}</option>)}
                    </select>
                  </div>
                  <div style={s.field}>
                    <label style={s.label}>Product Image</label>
                    <input style={s.input} type="file" onChange={handleEditImageUpload} />
                    {editPreview && <img src={editPreview} alt="preview" style={s.previewImg} />}
                  </div>
      ...
      <div style={s.main}>
          ...
          <div style={s.formCard}>
            ...
            <form onSubmit={handleSubmit}>
              <div style={s.formGrid}>
                ...
                <div style={s.field}>
                  <label style={s.label}>Discount Price (₦)</label>
                  <input style={s.input} type="number" name="discount_price" value={formData.discount_price} onChange={handleChange} placeholder="Optional" />
                </div>
                <div style={s.field}>
                  <label style={s.label}>Total Quantity in Stock</label>
                ...
                <div style={s.field}>
                  <label style={s.label}>Min Order Qty</label>
                  <input style={s.input} type="number" name="min_order_qty" value={formData.min_order_qty} onChange={handleChange} />
                </div>
                <div style={s.field}>
                  <label style={s.label}>Unit</label>
                  <select style={s.input} name="unit" value={formData.unit} onChange={handleChange}>
                    {['bag', 'kg', 'ton', 'crate', 'litre', 'piece', 'bunch'].map(u => <option key={u} value={u}>{u}</option>)}
                  </select>
                </div>
                <div style={s.field}>
        ...
        <div style={s.filtersRow}>
          ...
          <div style={s.filterCount}>{filtered.length} product{filtered.length !== 1 ? 's' : ''}</div>
        </div>
                ...
                return (
                  <tr key={p.id} style={s.tableRow}>
                    ...
                    <td style={s.td}>
                      {p.discount_price ? (
                        <div>
                          <div style={s.discountPrice}>₦{Number(p.discount_price).toLocaleString()}</div>
                          <div style={s.originalPrice}>₦{Number(p.price).toLocaleString()}</div>
                        </div>
                      ) : (
...
const s = {
  ...
  modalBox: { background: '#fff', borderRadius: 14, width: '100%', maxWidth: 780, maxHeight: '90vh', display: 'flex', flexDirection: 'column' },
  ...
  cancelBtn: { padding: '10px 24px', background: '#f5f5f5', color: '#333', border: '1px solid #ddd', borderRadius: 8, cursor: 'pointer', fontFamily: 'inherit' },
  ...
  previewImg: { width: 60, height: 60, marginTop: 8, borderRadius: 6, objectFit: 'cover', border: '1px solid #eee' },
  ...
  filtersRow: { display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16, flexWrap: 'wrap' },
  searchInput: { flex: 1, minWidth: 200, padding: '10px 16px', border: '1px solid #ddd', borderRadius: 8, fontSize: 14, fontFamily: 'inherit', outline: 'none' },
  filterSelect: { padding: '10px 14px', border: '1px solid #ddd', borderRadius: 8, fontSize: 13, fontFamily: 'inherit', outline: 'none', background: '#fff' },
  ...
  productThumb: { width: 44, height: 44, borderRadius: 6, objectFit: 'cover', flexShrink: 0 },
  ...
  remainingVal: { fontSize: 13, fontWeight: 600, color: '#111', marginBottom: 4 },
  stockBar: { width: '80px', height: 4, background: '#eee', borderRadius: 99, overflow: 'hidden' },
  ...
  editBtn: { background: '#f0f7ec', color: '#1f4d1f', border: '1px solid #a8d5a8', padding: '6px 14px', borderRadius: 6, cursor: 'pointer', fontSize: 12, fontWeight: 600, marginRight: 8, fontFamily: 'inherit' },
