...
function MiniBar({ data, color = "#1f4d1f" }) {
  ...
  return (
    <div
      style={{ display: "flex", alignItems: "flex-end", gap: 5, height: 80 }}
    >
      {data.map((d, i) => (
        <div
          key={i}
        ...
        >
          ...
          <div
            style={{
              ...
              height: `${Math.max((d.value / max) * 60, d.value > 0 ? 3 : 0)}px`,
              opacity: 0.8,
            }}
          />
...
export default function ManageSellersPage() {
  ...
  const handleSelectSeller = (seller) => {
    ...
    api
      .get(`/admin/sellers/${seller.id}/stats`)
  ...
  const handleRemit = async (sellerId) => {
    ...
    try {
      ...
      showToast(
        `Remitted ₦${Number(res.data.amount_remitted).toLocaleString()} to ${res.data.seller}`,
      );
                ...
                <div style={s.statsPanelBody}>
                    ...
                    {[
                      {
                        ...
                        value: sellerStats.overview.total_products,
                      },
                      {
                        label: "Active",
                        value: sellerStats.overview.active_products,
                      },
                      {
                        label: "Out of Stock",
                        value: sellerStats.overview.out_of_stock,
                      },
                      {
                        label: "Items Sold",
                        value: sellerStats.overview.total_items_sold,
                      },
                      {
                        label: "Delivered",
                        value: sellerStats.overview.delivered_items,
                      },
                      {
                        label: "Pending",
                        value: sellerStats.overview.pending_delivery,
                      },
                    ].map((item) => (
                  ...
                  {sellerStats.monthly_revenue?.length > 0 && (
                    <div style={s.chartBox}>
                      ...
                      <MiniBar
                        data={buildChartData(sellerStats.monthly_revenue)}
                        color="#1f4d1f"
                      />
                    </div>
                  )}
