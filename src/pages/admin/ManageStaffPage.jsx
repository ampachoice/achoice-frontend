        ...
        <div style={s.modalOverlay}>
          <div style={s.modalBox}>
            <div style={s.modalHeader}>
              ...
              <button style={s.modalClose} onClick={() => setEditingStaff(null)}>✕</button>
            </div>
                  ...
                  <div style={s.permissionsGrid}>
                    <label style={s.checkboxLabel}>
                      <input type="checkbox" style={s.checkbox}
                        ...
                        onChange={e => setEditingStaff({ ...editingStaff, can_manage_agro: e.target.checked })}
                      />
                      <div>
                        <div style={s.checkboxTitle}>Agro/Sales Dashboard</div>
                        <div style={s.checkboxSub}>Manage orders, inventory, reports</div>
                      </div>
                    </label>
                    <label style={s.checkboxLabel}>
                      <input type="checkbox" style={s.checkbox}
                        checked={editingStaff.can_manage_loans}
                        onChange={e => setEditingStaff({ ...editingStaff, can_manage_loans: e.target.checked })}
                      />
                      <div>
                        <div style={s.checkboxTitle}>Loan Dashboard</div>
                        <div style={s.checkboxSub}>Review applications, documents</div>
                      </div>
                    </label>
                  </div>
                </div>
      ...
      <div style={s.main}>
        ...
        {staff.length === 0 ? (
          <div style={s.emptyBox}>
            ...
            <p style={s.emptyText}>Add your first staff member to get started.</p>
          </div>
        ) : (
                  ...
                  <div style={s.permsRow}>
                    ...
                    <div style={s.permsTags}>
                      ...
                      {member.can_manage_loans && (
                        <span style={{ ...s.permTag, background: '#e7f0ff', color: '#1a4fa0' }}>Loans</span>
                      )}
                      {!member.can_manage_agro && !member.can_manage_loans && (
                        <span style={{ ...s.permTag, background: '#f0f0f0', color: '#888' }}>None</span>
                      )}
                    </div>
                  </div>
                    ...
                    <div style={s.editPermsRow}>
                      ...
                      <button
                        ...
                        onClick={() => handleUpdatePermissions(member.id, {
                          can_manage_agro: document.getElementById(`agro-${member.id}`).checked,
                          can_manage_loans: document.getElementById(`loans-${member.id}`).checked,
                        })}
                      >
                        Save
                      </button>
...
const s = {
  ...
  modalBox: { background: '#fff', borderRadius: 12, width: '100%', maxWidth: 520 },
  ...
  modalClose: { background: 'none', border: 'none', fontSize: 20, cursor: 'pointer', color: '#888' },
  modalBody: { padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 14 },
  modalFooter: { display: 'flex', justifyContent: 'flex-end', gap: 10, padding: '16px 24px', borderTop: '1px solid #eee' },
  cancelBtn: { padding: '10px 20px', background: '#f5f5f5', color: '#333', border: '1px solid #ddd', borderRadius: 6, cursor: 'pointer', fontFamily: 'inherit' },
  ...
  permissionsBox: { background: '#f7f5f0', borderRadius: 10, padding: 20, marginBottom: 20 },
  permissionsTitle: { fontSize: 13, fontWeight: 700, color: '#333', marginBottom: 14 },
  permissionsGrid: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 },
  checkboxLabel: { display: 'flex', alignItems: 'flex-start', gap: 10, padding: 14, background: '#fff', borderRadius: 8, border: '1px solid #e8e4dc', cursor: 'pointer' },
  checkbox: { marginTop: 2, width: 16, height: 16, flexShrink: 0 },
  checkboxTitle: { fontSize: 13, fontWeight: 600, color: '#111', marginBottom: 3 },
  ...
  staffCard: { background: '#fff', borderRadius: 12, border: '1px solid #e8e4dc', padding: 20 },
  staffCardHeader: { display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 16 },
  staffAvatar: { width: 44, height: 44, background: '#1f4d1f', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#f0c050', fontWeight: 700, fontSize: 18, flexShrink: 0 },
  staffInfo: { flex: 1 },
  ...
  statusBadge: { fontSize: 11, fontWeight: 600, padding: '4px 10px', borderRadius: 99, flexShrink: 0 },
  ...
  permTag: { fontSize: 11, fontWeight: 600, padding: '3px 8px', borderRadius: 6, background: '#eafaf0', color: '#1a7a3a' },
  editPerms: { background: '#f7f5f0', borderRadius: 8, padding: '12px 14px', marginBottom: 14 },
  editPermsTitle: { fontSize: 11, color: '#888', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 8 },
  editPermsRow: { display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap' },
  miniCheckLabel: { display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: '#333', cursor: 'pointer' },
  savePermsBtn: { padding: '5px 12px', background: '#1f4d1f', color: '#fff', border: 'none', borderRadius: 5, fontSize: 12, cursor: 'pointer', fontFamily: 'inherit' },
  cardActions: { display: 'flex', gap: 6, marginBottom: 12, flexWrap: 'wrap' },
  viewBtn: { flex: 1, padding: '8px 0', background: '#f0f7ec', color: '#1f4d1f', border: '1px solid #a8d5a8', borderRadius: 6, cursor: 'pointer', fontSize: 11, fontWeight: 600, fontFamily: 'inherit', textAlign: 'center' },
  editBtn: { padding: '8px 12px', background: '#e7f0ff', color: '#1a4fa0', border: '1px solid #b3d0ff', borderRadius: 6, cursor: 'pointer', fontSize: 11, fontFamily: 'inherit' },
  activateBtn: { padding: '8px 12px', background: '#1f4d1f', color: '#fff', border: 'none', borderRadius: 6, cursor: 'pointer', fontSize: 11, fontFamily: 'inherit' },
  deactivateBtn: { padding: '8px 12px', background: '#fff8e7', color: '#b36b00', border: '1px solid #f0c050', borderRadius: 6, cursor: 'pointer', fontSize: 11, fontFamily: 'inherit' },
  deleteBtn: { padding: '8px 12px', background: '#fff0f0', color: '#cc0000', border: '1px solid #ffa39e', borderRadius: 6, cursor: 'pointer', fontSize: 11, fontFamily: 'inherit' },
