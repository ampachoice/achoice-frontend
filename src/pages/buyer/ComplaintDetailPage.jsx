            ...
            <>
              ...
              <div className="cdp-card">
                <div className="cdp-card-top">
                  ...
                  <span className="cdp-badge" style={getStatusStyle(complaint.status)}>
                    {complaint.status
                      ? complaint.status.replace(/_/g, " ").charAt(0).toUpperCase() +
                        complaint.status.replace(/_/g, " ").slice(1)
                      : "Pending"}
                  </span>
                </div>
                <div className="cdp-meta">Type: {complaint.type?.replace(/_/g, " ")}</div>
                {complaint.order?.order_number && (
                  <div className="cdp-meta">Order: {complaint.order.order_number}</div>
                )}
                <div className="cdp-meta">
                  Submitted: {complaint.created_at
                    ? new Date(complaint.created_at).toLocaleDateString("en-NG", {
              ...
              <div className="cdp-card">
                ...
                <div className="cdp-messages">
                  {(!complaint.messages || complaint.messages.length === 0) ? (
                    ...
                    complaint.messages.map(msg => (
                      <div key={msg.id} className={`cdp-msg-row ${msg.sender_role}`}>
                        <div className={`cdp-bubble ${msg.sender_role}`}>
                          ...
                          {msg.attachment_url && (
                            <a href={msg.attachment_url} target="_blank" rel="noreferrer"
                              className="cdp-msg-attach">📎 View Attachment</a>
                          )}
                ...
                {complaint.status === "resolved" || complaint.status === "rejected" ? (
                  <div className="cdp-resolved-note">
                    This complaint has been {complaint.status}. No further messages can be sent.
                  </div>
                ) : (
                  <form onSubmit={sendMessage}>
                    <div className="cdp-input-row">
                      <input
                        ...
                        disabled={sending}
                      />
                      <button className="cdp-send-btn" type="submit" disabled={sending || !newMessage.trim()}>
                        {sending ? "..." : "Send"}
                      </button>
                    </div>
                  </form>
