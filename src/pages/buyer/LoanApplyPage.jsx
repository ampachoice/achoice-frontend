        ...
        <div style={s.successContainer}>
          <div style={s.successCard}>
            ...
            <p style={s.successText}>
              ...
              Our loan team will review everything and get back to you within 24 hours.
            </p>
            ...
            <div style={s.uploadStatusBox}>
              ...
              {Object.entries(uploadProgress).map(([key, status]) => (
                <div key={key} style={s.uploadStatusRow}>
                  <span style={s.uploadStatusLabel}>{key.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase())}</span>
                  <span style={{
                    ...s.uploadStatusBadge,
      ...
      <div style={s.modalBox} onClick={e => e.stopPropagation()}>
        ...
        <div style={s.modalBody}>
          {[
            ...
            { title: '4. Document Authenticity', text: 'All documents submitted (NIN, BVN, bank statement, collateral) must be authentic and belong to you. Submission of false documents is a criminal offence and will result in immediate cancellation of the loan and potential legal action.' },
            { title: '5. NIN & BVN Verification', text: 'Your National Identification Number (NIN) and Bank Verification Number (BVN) will be verified with the relevant government agencies. By applying, you consent to this verification.' },
            ...
            { title: '7. Privacy', text: 'Your personal information and documents will be stored securely and used only for loan processing purposes. We will not share your data with third parties without your consent, except where required by law.' },
        ...
        <div style={s.modalFooter}>
          <button style={s.modalAgreeBtn} onClick={() => { setAgreedToTerms(true); setShowTerms(false); }}>
            ✓ I Agree to These Terms
          </button>
          <button style={s.modalCloseBtn} onClick={() => setShowTerms(false)}>Close</button>
        </div>
      </div>
    </div>
  );
    ...
    <div style={s.page}>
      ...
      <div style={s.heroBanner}>
        <div style={s.heroContent}>
          ...
          <p style={s.heroSubtitle}>Quick, affordable loans for farmers and agro-businesses. Apply in minutes.</p>
        ...
        <div style={s.stepBar}>
          {[
            ...
            <div key={st.num} style={s.stepBarItem}>
              ...
              <div style={{ ...s.stepLabel, color: step >= st.num ? '#1f4d1f' : '#aaa', fontWeight: step === st.num ? 700 : 400 }}>
                {st.label}
              </div>
              {i < 3 && <div style={{ ...s.stepLine, background: step > st.num ? '#1f4d1f' : '#e8e4dc' }} />}
            </div>
          ))}
        </div>
            ...
            {step === 1 && (
              <div style={s.card}>
                <div style={s.cardHeader}>
                  ...
                  <p style={s.cardSub}>Tell us how much you need and what for.</p>
                </div>
                ...
                <div style={s.field}>
                  ...
                  <div style={s.hint}>Min: ₦{minAmount.toLocaleString()} · Max: ₦{maxAmount.toLocaleString()}</div>
                </div>

                <div style={s.field}>
                  <label style={s.label}>Purpose of Loan <span style={s.req}>*</span></label>
                  <select style={s.input} value={loanForm.purpose}
                ...
                <div style={s.field}>
                  <label style={s.label}>Loan Duration <span style={s.req}>*</span></label>
                  <div style={s.durationGrid}>
                    {durations.length > 0 ? durations.map(d => (
                      <button key={d.id} type="button"
                        style={{
                          ...s.durationBtn,
                          ...(String(loanForm.duration_months) === String(d.months) ? s.durationBtnActive : {}),
                        }}
                        onClick={() => setLoanForm(p => ({ ...p, duration_months: String(d.months) }))}>
                        {d.label}
                      </button>
                    )) : (
                      <div style={{ fontSize: 13, color: '#888' }}>Loading durations from loan settings...</div>
                    )}
                  </div>
                </div>

                <div style={s.field}>
                  <label style={s.label}>Repayment Preference <span style={s.req}>*</span></label>
                  <div style={s.radioGroup}>
                    {[
                      ...
                      <label key={opt.val} style={{
                        ...s.radioCard,
                        ...(loanForm.repayment_preference === opt.val ? s.radioCardActive : {}),
                      }}>
                  ...
                  <div style={s.summaryBox}>
                    ...
                    {[
                      ...
                      ...(loanForm.duration_months ? [
                        ['Monthly Instalment', `₦${calcMonthly.toLocaleString()}`],
                        ['Weekly Instalment',  `₦${calcWeekly.toLocaleString()}`],
                      ] : []),
                    ].map(([label, val]) => (
                ...
                <button style={s.nextBtn} onClick={nextStep}>Continue → Identity Verification</button>
              </div>
            )}
            ...
            {step === 2 && (
              <div style={s.card}>
                <div style={s.cardHeader}>
                  ...
                  <p style={s.cardSub}>Your NIN and BVN are required to verify your identity.</p>
                </div>

                <div style={s.infoNotice}>
                  🔒 Your NIN and BVN will be verified with government databases. This information is kept strictly confidential.
                </div>

                <div style={s.field}>
                  <label style={s.label}>National Identification Number (NIN) <span style={s.req}>*</span></label>
                  <input style={s.input} type="text"
                    value={identityForm.nin_number}
                    onChange={e => setIdentityForm(p => ({ ...p, nin_number: e.target.value.replace(/\D/g, '') }))}
                    placeholder="Enter your 11-digit NIN"
                    maxLength={11} />
                  <div style={s.hint}>
                    {identityForm.nin_number.length}/11 digits
                    {identityForm.nin_number.length === 11 && <span style={{ color: '#1a7a3a', marginLeft: 8 }}>✓ Valid length</span>}
                  </div>
                </div>

                <div style={s.field}>
                  <label style={s.label}>Bank Verification Number (BVN) <span style={s.req}>*</span></label>
                  <input style={s.input} type="text"
                    value={identityForm.bvn_number}
                    onChange={e => setIdentityForm(p => ({ ...p, bvn_number: e.target.value.replace(/\D/g, '') }))}
                    placeholder="Enter your 11-digit BVN"
                    maxLength={11} />
                  <div style={s.hint}>
                    {identityForm.bvn_number.length}/11 digits
                    {identityForm.bvn_number.length === 11 && <span style={{ color: '#1a7a3a', marginLeft: 8 }}>✓ Valid length</span>}
                  </div>
                  <div style={s.hint}>To get your BVN, dial <strong>*565*0#</strong> on any bank's line.</div>
                </div>

                <div style={s.btnRow}>
                  <button style={s.backBtn} onClick={prevStep}>← Back</button>
                  <button style={s.nextBtn} onClick={nextStep}>Continue → Documents</button>
                </div>
              </div>
            )}
            ...
            {step === 3 && (
              <div style={s.card}>
                <div style={s.cardHeader}>
                  ...
                  <p style={s.cardSub}>Upload clear scans or photos. Accepted: JPG, PNG, PDF. Max 5MB each.</p>
                </div>

                <div style={s.infoNotice}>
                  📎 NIN document, BVN document and bank statement are required. Collateral is optional but increases your chances of approval.
                </div>

                {[
                  { key: 'nin_document',        label: 'NIN Document',           req: true,  desc: 'Upload a clear photo or scan of your National ID card, NIMC slip or NIN slip', icon: '🪪' },
                  ...
                  <div key={doc.key} style={s.docField}>
                    <div style={s.docFieldHeader}>
                      ...
                      <div style={s.docFieldInfo}>
                        <div style={s.docFieldLabel}>
                          ...
                          {!doc.req && <span style={s.optional}> (optional)</span>}
                        </div>
                    ...
                    {documents[doc.key] && (
                      <div style={s.fileSelected}>
                        ...
                        <span style={s.fileSize}> ({(documents[doc.key].size / 1024).toFixed(1)} KB)</span>
                        <button style={s.fileRemove}
                          onClick={() => setDocuments(p => ({ ...p, [doc.key]: null }))}>✕</button>
                      </div>
                    )}
                  </div>
                ))}
                ...
                <div style={s.btnRow}>
                  <button style={s.backBtn} onClick={prevStep}>← Back</button>
                  <button style={s.nextBtn} onClick={nextStep}>Continue → Review</button>
                </div>
              </div>
            )}
            ...
            {step === 4 && (
              <div style={s.card}>
                <div style={s.cardHeader}>
                  ...
                  <p style={s.cardSub}>Review your application details before submitting.</p>
                </div>
                ...
                <div style={s.reviewSection}>
                  ...
                  <button style={s.editBtn} onClick={() => setStep(1)}>✏ Edit</button>
                </div>
                ...
                <div style={s.reviewSection}>
                  ...
                  <div style={s.reviewGrid}>
                    <div style={s.reviewItem}>
                      ...
                      <div style={s.reviewVal}>{'•'.repeat(7) + identityForm.nin_number.slice(-4)}</div>
                    </div>
                    <div style={s.reviewItem}>
                      <div style={s.reviewLabel}>BVN</div>
                      <div style={s.reviewVal}>{'•'.repeat(7) + identityForm.bvn_number.slice(-4)}</div>
                    </div>
                  </div>
                  <button style={s.editBtn} onClick={() => setStep(2)}>✏ Edit</button>
                </div>
                ...
                <div style={s.reviewSection}>
                  ...
                  {[
                    ...
                    <div key={doc.key} style={s.docReviewRow}>
                      ...
                      {documents[doc.key]
                        ? <span style={s.docReviewDone}>✅ {documents[doc.key].name}</span>
                        : <span style={s.docReviewNone}>Not uploaded</span>
                      }
                    </div>
                  ))}
                  <button style={s.editBtn} onClick={() => setStep(3)}>✏ Edit</button>
                </div>
                ...
                <div style={s.termsSection}>
                  <label style={s.termsLabel}>
                    ...
                    <div style={s.termsText}>
                      ...
                      <button style={s.termsLink} type="button" onClick={() => setShowTerms(true)}>
                        Terms and Conditions
                      </button>
                ...
                <div style={s.btnRow}>
                  <button style={s.backBtn} onClick={prevStep}>← Back</button>
                  <button
                    style={submitting || !agreedToTerms ? s.submitBtnDisabled : s.submitBtn}
            ...
            <div style={s.stepGuide}>
              ...
              {[
                ...
                <div key={st.n} style={s.stepGuideItem}>
                  <div style={{ ...s.stepGuideNum, background: step >= st.n ? '#1f4d1f' : '#e8e4dc', color: step >= st.n ? '#fff' : '#aaa' }}>
                    {st.done ? '✓' : st.n}
                  </div>
                  <div style={{ ...s.stepGuideLabel, color: step >= st.n ? '#111' : '#aaa', fontWeight: step === st.n ? 700 : 400 }}>{st.label}</div>
                </div>
              ))}
            </div>
          </div>
...
const s = {
  ...
  cartBadge: { position: 'absolute', top: -6, right: -8, background: '#f0c050', color: '#1f4d1f', fontSize: 10, fontWeight: 700, borderRadius: '50%', width: 16, height: 16, display: 'flex', alignItems: 'center', justifyContent: 'center' },

  // Hero
  heroBanner: { background: '#1a3d1a', padding: '48px 60px' },
  heroContent: { maxWidth: 900, margin: '0 auto' },
  heroBadge: { display: 'inline-block', background: '#f0c050', color: '#1a3d1a', fontSize: 11, fontWeight: 700, padding: '4px 14px', borderRadius: 99, marginBottom: 16 },
  ...
  heroStatDivider: { position: 'absolute', right: -16, top: 0, bottom: 0, width: 1, background: 'rgba(255,255,255,0.2)' },

  // Step bar
  stepBar: { display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '28px 0', gap: 0 },
  stepBarItem: { display: 'flex', alignItems: 'center', gap: 8, position: 'relative' },
  ...
  // Card
  card: { background: '#fff', borderRadius: 12, border: '1px solid #e8e4dc', padding: 32, marginBottom: 16 },
  cardHeader: { marginBottom: 24 },
  cardStep: { fontSize: 11, fontWeight: 700, color: '#1f4d1f', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 6 },
  cardTitle: { fontSize: 22, fontWeight: 700, color: '#111', marginBottom: 6 },
  cardSub: { fontSize: 13, color: '#888' },

  // Fields
  field: { marginBottom: 22 },
  label: { display: 'block', fontSize: 13, fontWeight: 600, color: '#333', marginBottom: 7 },
  req: { color: '#cc0000' },
  optional: { fontSize: 11, color: '#aaa', fontWeight: 400 },
  input: { width: '100%', padding: '12px 14px', border: '1px solid #ddd', borderRadius: 8, fontSize: 14, fontFamily: 'inherit', outline: 'none', boxSizing: 'border-box' },
  ...
  radioCard: { flex: 1, padding: '14px 16px', border: '1px solid #ddd', borderRadius: 8, cursor: 'pointer', textAlign: 'center' },
  ...
  // Summary
  summaryBox: { background: '#f0f7ec', border: '1px solid #c5ddb8', borderRadius: 10, padding: 18, marginBottom: 20 },
  ...
  // Info notice
  infoNotice: { background: '#fff8e7', border: '1px solid #f0c050', borderRadius: 8, padding: '12px 16px', fontSize: 13, color: '#7a5c00', marginBottom: 20 },

  // Document fields
  docField: { background: '#f7f5f0', borderRadius: 10, padding: 16, marginBottom: 14 },
  docFieldHeader: { display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 10 },
  docFieldIcon: { fontSize: 24, flexShrink: 0 },
  docFieldInfo: { flex: 1 },
  docFieldLabel: { fontSize: 14, fontWeight: 600, color: '#111', marginBottom: 3 },
  docFieldDesc: { fontSize: 12, color: '#888', lineHeight: 1.5 },
  fileInput: { width: '100%', padding: '8px', border: '1px dashed #ccc', borderRadius: 7, fontSize: 13, background: '#fff', cursor: 'pointer', boxSizing: 'border-box' },
  fileSelected: { display: 'flex', alignItems: 'center', gap: 8, marginTop: 8, fontSize: 12, color: '#1a7a3a', background: '#eafaf0', padding: '6px 10px', borderRadius: 6 },
  fileSize: { color: '#888' },
  fileRemove: { background: 'none', border: 'none', cursor: 'pointer', color: '#cc0000', fontSize: 14, marginLeft: 'auto' },

  // Review
  reviewSection: { background: '#f7f5f0', borderRadius: 10, padding: 16, marginBottom: 16, position: 'relative' },
  reviewSectionTitle: { fontSize: 13, fontWeight: 700, color: '#1f4d1f', marginBottom: 12, textTransform: 'uppercase', letterSpacing: 0.5 },
  reviewGrid: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 8 },
  reviewItem: {},
  reviewLabel: { fontSize: 11, color: '#888', marginBottom: 2 },
  reviewVal: { fontSize: 13, fontWeight: 600, color: '#111', textTransform: 'capitalize' },
  editBtn: { background: 'none', border: '1px solid #ddd', borderRadius: 5, padding: '4px 12px', fontSize: 11, color: '#555', cursor: 'pointer', fontFamily: 'inherit' },
  docReviewRow: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '7px 0', borderBottom: '1px solid #eee', fontSize: 13 },
  docReviewLabel: { color: '#555' },
  docReviewDone: { color: '#1a7a3a', fontSize: 12 },
  docReviewNone: { color: '#aaa', fontSize: 12 },

  // Terms
  termsSection: { background: '#f0f7ec', border: '1px solid #c5ddb8', borderRadius: 10, padding: 16, marginBottom: 20 },
  termsLabel: { display: 'flex', alignItems: 'flex-start', gap: 12, cursor: 'pointer' },
  termsCheckbox: { marginTop: 2, width: 18, height: 18, flexShrink: 0, cursor: 'pointer' },
  termsText: { fontSize: 13, color: '#333', lineHeight: 1.6 },
  termsLink: { background: 'none', border: 'none', color: '#1f4d1f', fontWeight: 700, fontSize: 13, cursor: 'pointer', textDecoration: 'underline', padding: 0, fontFamily: 'inherit' },

  // Errors
  errorBanner: { background: '#fff0f0', border: '1px solid #ffb3b3', borderRadius: 8, padding: '12px 16px', fontSize: 13, color: '#cc0000', marginBottom: 16 },

  // Buttons
  btnRow: { display: 'flex', gap: 12 },
  nextBtn: { flex: 1, padding: '13px', background: '#1f4d1f', color: '#fff', border: 'none', borderRadius: 8, fontSize: 15, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' },
  backBtn: { padding: '13px 20px', background: '#fff', color: '#555', border: '1px solid #ddd', borderRadius: 8, fontSize: 14, cursor: 'pointer', fontFamily: 'inherit' },
  submitBtn: { flex: 1, padding: '13px', background: '#1f4d1f', color: '#fff', border: 'none', borderRadius: 8, fontSize: 15, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' },
  submitBtnDisabled: { flex: 1, padding: '13px', background: '#ccc', color: '#fff', border: 'none', borderRadius: 8, fontSize: 15, cursor: 'not-allowed', fontFamily: 'inherit' },

  // Right panel
  infoCol: { display: 'flex', flexDirection: 'column', gap: 16, position: 'sticky', top: 80 },
  infoCard: { background: '#1f4d1f', borderRadius: 12, padding: 22 },
  infoCardTitle: { fontSize: 14, fontWeight: 700, color: '#fff', marginBottom: 16 },
  infoItem: { display: 'flex', alignItems: 'flex-start', gap: 10, marginBottom: 12 },
  infoIcon: { color: '#f0c050', fontWeight: 700, fontSize: 14, flexShrink: 0 },
  infoText: { color: '#a8d5a8', fontSize: 13, lineHeight: 1.5 },
  termsCard: { background: '#fff', borderRadius: 12, border: '1px solid #e8e4dc', padding: 18 },
  termsCardTitle: { fontSize: 13, fontWeight: 700, color: '#111', marginBottom: 14 },
  termsRow: { display: 'flex', justifyContent: 'space-between', marginBottom: 10 },
  termsRowLabel: { fontSize: 12, color: '#888' },
  termsRowVal: { fontSize: 12, fontWeight: 600, color: '#1f4d1f' },
  stepGuide: { background: '#fff', borderRadius: 12, border: '1px solid #e8e4dc', padding: 18 },
  stepGuideTitle: { fontSize: 13, fontWeight: 700, color: '#111', marginBottom: 16 },
  stepGuideItem: { display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 },
  stepGuideNum: { width: 24, height: 24, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, flexShrink: 0 },
  ...
  successCard: { background: '#fff', borderRadius: 12, border: '1px solid #e8e4dc', padding: 40, maxWidth: 480, width: '100%', textAlign: 'center' },
  successIconCircle: { width: 72, height: 72, background: '#1f4d1f', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 32, color: '#fff', margin: '0 auto 20px' },
  successTitle: { fontSize: 22, fontWeight: 700, color: '#111', marginBottom: 12 },
  successText: { fontSize: 14, color: '#666', lineHeight: 1.7, marginBottom: 24 },
  uploadStatusBox: { background: '#f7f5f0', borderRadius: 8, padding: 16, marginBottom: 24, textAlign: 'left' },
  uploadStatusTitle: { fontSize: 12, fontWeight: 700, color: '#555', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 12 },
  uploadStatusRow: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 },
  uploadStatusLabel: { fontSize: 13, color: '#333', textTransform: 'capitalize' },
  uploadStatusBadge: { fontSize: 11, fontWeight: 600, padding: '3px 10px', borderRadius: 99 },
  successBtn: { width: '100%', padding: '13px', background: '#1f4d1f', color: '#fff', border: 'none', borderRadius: 8, fontSize: 15, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', marginBottom: 10 },
  outlineBtn: { width: '100%', padding: '13px', background: '#fff', color: '#1f4d1f', border: '2px solid #1f4d1f', borderRadius: 8, fontSize: 15, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' },

  // Terms Modal
  modalOverlay: { position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 },
  modalBox: { background: '#fff', borderRadius: 14, width: '100%', maxWidth: 600, maxHeight: '85vh', display: 'flex', flexDirection: 'column', boxShadow: '0 20px 60px rgba(0,0,0,0.3)' },
  modalHeader: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px 24px', background: '#1f4d1f', borderRadius: '14px 14px 0 0', flexShrink: 0 },
  ...
  termTitle: { fontSize: 14, fontWeight: 700, color: '#1f4d1f', marginBottom: 6 },
  termText: { fontSize: 13, color: '#555', lineHeight: 1.7 },
  modalFooter: { display: 'flex', gap: 12, padding: '16px 24px', borderTop: '1px solid #eee', flexShrink: 0 },
  modalAgreeBtn: { flex: 1, padding: '11px', background: '#1f4d1f', color: '#fff', border: 'none', borderRadius: 8, fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' },
  modalCloseBtn: { padding: '11px 20px', background: '#f5f5f5', color: '#555', border: '1px solid #ddd', borderRadius: 8, fontSize: 14, cursor: 'pointer', fontFamily: 'inherit' },
};