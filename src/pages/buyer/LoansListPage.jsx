...
export default function LoansListPage() {
  ...
  const handleLiquidateAll = async () => {
    if (!window.confirm('Pay off the full remaining balance of every active loan right now, in one payment?')) return;
  ...
  return (
    <div style={s.page}>
      <style>{`
        .ll-nav { background:#1f4d1f; padding:12px 40px; display:flex; justify-content:space-between; align-items:center; color:#fff; position:sticky; top:0; z-index:100; gap:16px; }
        .ll-nav-left { display:flex; align-items:center; gap:12px; cursor:pointer; flex-shrink:0; }
        .ll-nav-links { display:flex; gap:24px; align-items:center; flex-shrink:1; min-width:0; overflow:hidden; }
        ...
        .ll-hamburger { display:none; background:none; border:none; font-size:22px; color:#fff; cursor:pointer; padding:4px; }
        .ll-drawer-overlay { position:fixed; inset:0; background:rgba(0,0,0,0.5); z-index:998; }
        .ll-drawer { position:fixed; top:0; right:0; bottom:0; width:75%; max-width:280px; background:#fff; z-index:999; display:flex; flex-direction:column; box-shadow:-4px 0 20px rgba(0,0,0,0.2); }
        .ll-drawer-header { background:#1f4d1f; padding:16px 20px; display:flex; justify-content:space-between; align-items:center; color:#fff; font-weight:700; }
        .ll-drawer-close { background:none; border:none; color:#fff; font-size:20px; cursor:pointer; }
        .ll-drawer-link { padding:16px 20px; font-size:15px; color:#111; cursor:pointer; border-bottom:1px solid #f0f0f0; }
      ...
      <nav className="ll-nav">
        ...
        <div className="ll-nav-right">
          ...
          <button className="ll-hamburger" onClick={() => setMenuOpen(true)} aria-label="Menu">☰</button>
        </div>
      </nav>

      {menuOpen && (
        <div className="ll-drawer-overlay" onClick={() => setMenuOpen(false)}>
          <div className="ll-drawer" onClick={(e) => e.stopPropagation()}>
            <div className="ll-drawer-header">
              <span>Menu</span>
              <button className="ll-drawer-close" onClick={() => setMenuOpen(false)}>✕</button>
            </div>
            <div className="ll-drawer-link" onClick={() => { setMenuOpen(false); navigate('/'); }}>Home</div>
            <div className="ll-drawer-link" onClick={() => { setMenuOpen(false); navigate('/loans/apply'); }}>Apply for Loan</div>
            <div className="ll-drawer-link" onClick={() => { setMenuOpen(false); navigate('/orders'); }}>My Orders</div>
          </div>
        </div>
      )}
