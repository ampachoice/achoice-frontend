...
export default function ProductPage() {
  ...
  useEffect(() => {
    ...
    getAllProducts()
      .then(res => {
        ...
        setAllProducts(data); setFilteredProducts(data);
      })
  ...
  // ── LOADING / ERROR ───────────────────────────────────────────────────────
  if (!id && listLoading) return <div className="pp-wrap"><Navbar /><div style={{ textAlign:'center', color:'#888', padding:80, fontSize:16 }}>Loading products...</div></div>;
  if (!id && listError)   return <div className="pp-wrap"><Navbar /><div style={{ textAlign:'center', color:'#cc0000', padding:80, fontSize:14 }}>{listError}</div></div>;
  if (id && detailLoading) return <div className="pp-wrap"><Navbar /><div style={{ textAlign:'center', color:'#888', padding:80, fontSize:16 }}>Loading product...</div></div>;
  if (id && !product && !detailLoading) return <div className="pp-wrap"><Navbar /><div style={{ textAlign:'center', color:'#888', padding:80, fontSize:16 }}>Product not found.</div></div>;
                ...
                <div className="pp-review-form">
                  ...
                  <form onSubmit={handleReviewSubmit}>
                    <div className="pp-review-field">
                      ...
                      <select className="pp-review-select" value={reviewForm.rating}
                        onChange={e => setReviewForm({ ...reviewForm, rating:Number(e.target.value) })}>
                        {[5,4,3,2,1].map(n => <option key={n} value={n}>{n} Star{n>1?'s':''} {'★'.repeat(n)}</option>)}
                      </select>
                    </div>
                    <div className="pp-review-field">
                      <label className="pp-review-label">Order ID <span style={{ color:'#aaa', fontWeight:400 }}>(optional)</span></label>
                      <input className="pp-review-input" type="text" placeholder="Your order ID"
