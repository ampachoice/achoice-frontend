...
const CATEGORIES = [
  "grains", "vegetables", "fruits", "tubers", "livestock",
  "poultry", "fishery", "dairy", "processed", "other",
];
...
const EMPTY_FORM = {
  ...
  quantity: "", min_order_qty: "1", unit: "bag", category: "", tags: "",
};
...
export default function SellerProductsPage() {
  ...
  useEffect(() => {
    getSellerProfile().then((res) => setSeller(res.data)).catch(() => {});
  }, []);

  const loadProducts = useCallback(() => {
    setLoading(true);
    getSellerProducts({ ...(statusFilter && { status: statusFilter }), ...(search && { search }) })
    ...
    try {
      ...
      if (imageFile) {
        ...
        try {
          ...
          setFormError("Image upload failed — the rest of the product was not saved. Please try again.");
  ...
  return (
    <SellerLayout
      ...
      headerActions={
        canAddProduct ? (
          <button style={s.newBtn} onClick={openCreateForm}>+ Add Product</button>
        ) : seller ? (
          <span style={s.disabledHint} title="Your seller account must be active to list products">
    ...
    >
      ...
      {loading ? (
        ...
        <div style={s.emptyState}>
          {statusFilter ? `No products with status "${statusFilter.replace(/_/g, " ")}".` : "You haven't listed any products yet."}
        </div>
      ) : (
        <div style={s.grid}>
          {products.map((p) => {
            const colors = STATUS_COLORS[p.status] || { bg: "#eee", color: "#555" };
            return (
              <div key={p.id} style={s.card}>
                <div style={s.cardImage}>
                  {p.image ? <img src={p.image} alt={p.name} style={s.cardImageImg} /> : <span style={{ fontSize: 34 }}>🌿</span>}
                ...
                <div style={{ padding: 14 }}>
                  ...
                  <div style={s.cardMeta}>{p.category} · {p.quantity} {p.unit} in stock</div>
                  <div style={{ display: "flex", alignItems: "baseline", gap: 6, margin: "6px 0 10px" }}>
                    {p.discount_price ? (
                      <>
                        <span style={{ fontWeight: 700, color: "#cc0000" }}>₦{Number(p.discount_price).toLocaleString()}</span>
                        <span style={{ fontSize: 12, color: "#999", textDecoration: "line-through" }}>₦{Number(p.price).toLocaleString()}</span>
                      </>
                    ) : (
                      <span style={{ fontWeight: 700, color: "#111" }}>₦{Number(p.price).toLocaleString()}</span>
                    )}
                  </div>
                  ...
                  <div style={{ display: "flex", gap: 10, marginTop: 10 }}>
                    <button style={s.linkBtn} onClick={() => openEditForm(p)}>Edit</button>
                    <button style={{ ...s.linkBtn, color: "#cc0000" }} onClick={() => handleDelete(p)}>Delete</button>
                  </div>
                </div>
              </div>
            );
      ...
      {showForm && (
        <div style={s.modalOverlay} onClick={closeForm}>
          <div style={s.modal} onClick={(e) => e.stopPropagation()}>
            <div style={s.modalTitle}>{editingProduct ? "Edit Product" : "Add Product"}</div>

            {formError && <div style={s.formError}>⚠️ {formError}</div>}
            {editingProduct?.status === "rejected" && editingProduct?.rejection_reason && (
              <div style={s.rejectionBanner}>
                <strong>This product was rejected:</strong> {editingProduct.rejection_reason}
              </div>
            )}
            ...
            <form onSubmit={handleSubmit}>
              <div style={s.field}>
                ...
                <div style={s.imageRow}>
                  <div style={s.imagePreviewBox}>
                    {imagePreview ? <img src={imagePreview} alt="Preview" style={s.imagePreviewImg} /> : <span style={{ fontSize: 24 }}>🌿</span>}
                  </div>
              ...
              <div style={s.field}>
                ...
                <input style={s.input} required value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} />
              </div>
              ...
              <div style={s.row2}>
                <div style={s.field}>
                  ...
                  <select style={s.input} required value={form.category} onChange={(e) => setForm((f) => ({ ...f, category: e.target.value }))}>
              ...
              <div style={s.row2}>
                <div style={s.field}>
                  ...
                  <input style={s.input} type="number" min="0" step="0.01" required value={form.price} onChange={(e) => setForm((f) => ({ ...f, price: e.target.value }))} />
                </div>
                <div style={s.field}>
                  <label style={s.label}>Discount Price (optional)</label>
                  <input style={s.input} type="number" min="0" step="0.01" value={form.discount_price} onChange={(e) => setForm((f) => ({ ...f, discount_price: e.target.value }))} />
                </div>
              </div>
              ...
              <div style={s.row2}>
                <div style={s.field}>
                  ...
                  <input style={s.input} type="number" min="0" required value={form.quantity} onChange={(e) => setForm((f) => ({ ...f, quantity: e.target.value }))} />
                </div>
                <div style={s.field}>
                  <label style={s.label}>Minimum Order Qty</label>
                  <input style={s.input} type="number" min="1" value={form.min_order_qty} onChange={(e) => setForm((f) => ({ ...f, min_order_qty: e.target.value }))} />
                </div>
              </div>

              <div style={s.field}>
                <label style={s.label}>Tags (optional, comma-separated)</label>
                <input style={s.input} value={form.tags} onChange={(e) => setForm((f) => ({ ...f, tags: e.target.value }))} placeholder="fresh, organic, local" />
              </div>
                  ...
                  <div style={s.galleryGrid}>
                    {galleryImages.map((img) => (
                      <div key={img.id} style={s.galleryThumb}>
                        <img src={img.image_url} alt="" style={s.galleryThumbImg} />
                        <button type="button" style={s.galleryRemoveBtn} onClick={() => handleDeleteGalleryImage(img.id)}>✕</button>
                      </div>
                    ))}
                    {galleryImages.length < 5 && (
                      <label style={s.galleryAddBtn}>
                        {galleryUploading ? "..." : "+ Add"}
                        <input type="file" accept="image/*" multiple onChange={handleAddGalleryImages} style={{ display: "none" }} disabled={galleryUploading} />
                      </label>
                    )}
                  </div>
                </div>
              ...
              {!editingProduct && (
                <div style={s.hint}>You'll be able to add gallery images right after creating this product.</div>
              )}
              ...
              <div style={s.modalActions}>
                ...
                <button type="submit" style={saving ? s.saveBtnDisabled : s.saveBtn} disabled={saving}>
                  {saving ? "Saving..." : editingProduct ? "Save Changes" : "Submit for Review"}
                </button>
              </div>
            </form>
          </div>
...
const s = {
  ...
  disabledHint: { fontSize: 12.5, color: "#a86a00", background: "#fff8e7", border: "1px solid #f0c050", borderRadius: 8, padding: "10px 14px" },
  ...
  controlsRow: { display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, marginBottom: 18, flexWrap: "wrap" },
  ...
  searchInput: { padding: "9px 14px", border: "1.5px solid #ddd", borderRadius: 20, fontSize: 13, minWidth: 220, fontFamily: "inherit" },
  ...
  card: { background: "#fff", border: "1px solid #e8e4dc", borderRadius: 12, overflow: "hidden" },
  cardImage: { position: "relative", height: 140, background: "#f7f5f0", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" },
  cardImageImg: { width: "100%", height: "100%", objectFit: "cover" },
  statusBadge: { position: "absolute", top: 8, left: 8, fontSize: 10, fontWeight: 700, padding: "3px 9px", borderRadius: 20, textTransform: "capitalize" },
  cardName: { fontSize: 13.5, fontWeight: 700, color: "#111", marginBottom: 4, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" },
  cardMeta: { fontSize: 11.5, color: "#888", textTransform: "capitalize" },
  rejectionNote: { fontSize: 11, color: "#a81f1f", background: "#fbe9e9", borderRadius: 6, padding: "6px 8px", marginTop: 6 },
  ...
  rejectionBanner: { background: "#fbe9e9", color: "#a81f1f", border: "1px solid #f3b3b3", borderRadius: 8, padding: "10px 14px", fontSize: 12.5, marginBottom: 16 },
  ...
  imagePreviewBox: { width: 64, height: 64, borderRadius: 8, background: "#f7f5f0", border: "1px solid #e8e4dc", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden", flexShrink: 0 },
  imagePreviewImg: { width: "100%", height: "100%", objectFit: "cover" },
  uploadBtn: { padding: "9px 16px", background: "#f7f5f0", border: "1.5px solid #ddd", borderRadius: 8, fontSize: 12.5, fontWeight: 600, color: "#1f4d1f", cursor: "pointer" },
  galleryGrid: { display: "flex", gap: 10, flexWrap: "wrap" },
  galleryThumb: { position: "relative", width: 64, height: 64, borderRadius: 8, overflow: "hidden", border: "1px solid #e8e4dc" },
  galleryThumbImg: { width: "100%", height: "100%", objectFit: "cover" },
  galleryRemoveBtn: { position: "absolute", top: 2, right: 2, width: 18, height: 18, borderRadius: "50%", background: "rgba(0,0,0,0.6)", color: "#fff", border: "none", fontSize: 10, cursor: "pointer", lineHeight: 1 },
  galleryAddBtn: { width: 64, height: 64, borderRadius: 8, border: "1.5px dashed #ccc", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, color: "#888", cursor: "pointer", textAlign: "center" },
