      ...
      <div style={s.main}>
        ...
        {activeTab === 'dashboard' && (
          <div>
            ...
            <p style={s.pageSub}>Welcome back, {user?.name?.split(' ')[0]}. Here's today's overview.</p>
            {stats ? (
              <>
  ...
  { label: "Total Revenue", value: `₦${Number(stats.revenue?.total || 0).toLocaleString()}`, icon: '💰', color: '#1a7a3a' },
                ...
                {stats.low_stock_count > 0 && (
                  <div style={s.alertBox}>
                    ...
                    <div style={s.alertText}>
                      ...
                      <span style={s.alertLink} onClick={() => setActiveTab('inventory')}> View Inventory</span>
                    </div>
                  </div>
                )}
              </>
        ...
        {activeTab === 'orders' && (
          <div>
            ...
            <div style={s.controlsRow}>
              <div style={s.filterTabs}>
                {['all', 'pending', 'processing', 'shipped', 'delivered', 'cancelled'].map(tab => (
                  <button key={tab}
                    style={orderFilter === tab ? s.filterTabActive : s.filterTab}
                ...
                <div key={order.id} style={{ ...s.orderCard, ...(isUnpaid ? s.orderCardUnpaid : {}) }}>
                  <div style={s.orderHeader}>
                    ...
                    <div style={s.orderRight}>
                      <div style={{ ...s.statusBadge, ...getStatusStyle(order.status) }}>{order.status}</div>
                      <div style={{
                        ...s.paymentBadge,
                      ...
                      <div style={s.orderTotal}>{toMoney(order.total_amount || order.total)}</div>
                    </div>
                  </div>

                  {isUnpaid && (
                    <div style={s.unpaidBanner}>
                      <div style={s.unpaidBannerText}>Payment not verified — buyer may have paid via Paystack</div>
                      <button
                    ...
                    <div style={s.orderItems}>
                      {order.items.map((item, i) => (
                        <div key={i} style={s.orderItem}>
                          <span style={s.itemName}>{item.product_name || item.product?.name}</span>
                  ...
                  <div style={s.actionRow}>
                    {order.status === 'pending' && order.payment_status === 'paid' && (
                      <button style={updating === order.id ? s.actionBtnDisabled : s.actionBtn}
                    ...
                    {order.status === 'shipped' && (
                      <div style={s.shippedNote}>Waiting for buyer to confirm delivery</div>
                    )}
                    {order.status === 'delivered' && (
                      <div style={s.deliveredNote}>Order delivered and completed</div>
                    )}
                    {(order.status === 'pending' || order.status === 'processing') && (
                      <button style={updating === order.id ? s.cancelBtnDisabled : s.cancelBtn}
            ...
            {!ordersLoading && filteredOrders.length === 0 && (
              <div style={s.empty}>No {orderFilter === 'all' ? '' : orderFilter} orders found.</div>
            )}
          </div>
        )}
        ...
        {activeTab === 'inventory' && (
          <div>
            ...
            <input
              style={{ ...s.searchInput, width: '100%', maxWidth: 400, marginBottom: 16 }}
              type="text"
            ...
            {lowStock.length > 0 && (
              <div style={s.alertBox}>
                <div style={s.alertTitle}>⚠️ Low Stock Products ({lowStock.length})</div>
                <div style={s.lowStockGrid}>
                  {lowStock.map(p => (
                    <div key={p.id} style={s.lowStockItem}>
                      <div style={s.lowStockName}>{p.name}</div>
                      <div style={s.lowStockQty}>{p.quantity} {p.unit} left</div>
                    </div>
                  ))}
                </div>
              </div>
                  ...
                  {filteredInventory.map(p => (
                    <tr key={p.id} style={s.tableRow}>
                      <td style={s.td}>
                        ...
                        <div style={s.productSeller}>{p.seller?.business_name}</div>
                      </td>
                      ...
                      <td style={s.td}>{p.items_sold || p.reviews_count || 0}</td>
                      <td style={s.td}>
                        <span style={{
                          ...s.statusBadge,
                  ...
                  salesReport.map((p, i) => (
                    <div key={p.id || i} style={s.reportRow}>
                      ...
                      <div style={s.reportInfo}>
                        ...
                        <div style={s.reportMeta}>{p.category} · {p.items_sold || 0} sold</div>
                      </div>
                      <div style={s.reportValue}>{toMoney(p.revenue || p.total_revenue)}</div>
                    </div>
                  ))
                )}
              </div>
                    ...
                    return (
                      <div key={i} style={s.revenueRow}>
                        ...
                        <div style={s.revenueBarBg}>
                          <div style={{ ...s.revenueBarFill, width: `${pct}%` }} />
                        </div>
                        <div style={s.revenueVal}>{toMoney(r.revenue || r.total)}</div>
                      </div>
                    );
                  })
                )}
...
const s = {
  ...
  toast: { position: 'fixed', top: 20, right: 20, background: '#1f4d1f', color: '#fff', padding: '12px 24px', borderRadius: 8, fontSize: 14, zIndex: 999 },
  ...
  alertBox: { background: '#fff8e7', border: '1px solid #f0c050', borderRadius: 10, padding: 16, marginBottom: 20 },
  alertTitle: { fontSize: 14, fontWeight: 700, color: '#b36b00', marginBottom: 6 },
  alertText: { fontSize: 13, color: '#7a5c00' },
  alertLink: { color: '#1f4d1f', fontWeight: 600, cursor: 'pointer', textDecoration: 'underline' },
  ...
  orderCard: { background: '#fff', borderRadius: 10, border: '1px solid #e8e4dc', padding: 20, marginBottom: 14 },
  ...
  orderRight: { textAlign: 'right', display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4 },
  ...
  unpaidBanner: { background: '#fff8e7', border: '1px solid #f0c050', borderRadius: 8, padding: '10px 14px', marginBottom: 12, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 },
  unpaidBannerText: { fontSize: 13, color: '#b36b00', flex: 1 },
  verifyBtn: { padding: '7px 16px', background: '#1f4d1f', color: '#fff', border: 'none', borderRadius: 6, fontSize: 12, cursor: 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap' },
  verifyBtnDisabled: { padding: '7px 16px', background: '#ccc', color: '#fff', border: 'none', borderRadius: 6, fontSize: 12, cursor: 'not-allowed', fontFamily: 'inherit' },
  orderItems: { borderTop: '1px solid #eee', padding: '10px 0', marginBottom: 10 },
  orderItem: { display: 'flex', gap: 10, alignItems: 'center', marginBottom: 6 },
  ...
  actionBtn: { padding: '8px 18px', background: '#1f4d1f', color: '#fff', border: 'none', borderRadius: 6, fontSize: 13, cursor: 'pointer', fontFamily: 'inherit' },
  actionBtnDisabled: { padding: '8px 18px', background: '#ccc', color: '#fff', border: 'none', borderRadius: 6, fontSize: 13, cursor: 'not-allowed', fontFamily: 'inherit' },
  cancelBtn: { padding: '8px 18px', background: '#fff', color: '#cc0000', border: '1px solid #cc0000', borderRadius: 6, fontSize: 13, cursor: 'pointer', fontFamily: 'inherit' },
  cancelBtnDisabled: { padding: '8px 18px', background: '#fff', color: '#ccc', border: '1px solid #ccc', borderRadius: 6, fontSize: 13, cursor: 'not-allowed', fontFamily: 'inherit' },
  shippedNote: { background: '#e7f0ff', color: '#1a4fa0', padding: '8px 14px', borderRadius: 6, fontSize: 13 },
  deliveredNote: { background: '#eafaf0', color: '#1a7a3a', padding: '8px 14px', borderRadius: 6, fontSize: 13 },
  empty: { textAlign: 'center', color: '#888', padding: 40 },
  lowStockGrid: { display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10, marginTop: 10 },
  lowStockItem: { background: '#fff', borderRadius: 6, padding: '10px 12px', border: '1px solid #f0c050' },
  lowStockName: { fontSize: 12, fontWeight: 600, color: '#111', marginBottom: 4 },
  ...
  reportCard: { background: '#fff', borderRadius: 12, border: '1px solid #e8e4dc', padding: 24 },
  reportTitle: { fontSize: 16, fontWeight: 700, color: '#111', marginBottom: 16 },
  reportRow: { display: 'flex', alignItems: 'center', gap: 10, padding: '10px 0', borderBottom: '1px solid #f5f5f5' },
  ...
  revenueRow: { display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 },
  revenueMonth: { fontSize: 12, color: '#555', width: 40 },
  revenueBarBg: { flex: 1, height: 8, background: '#eee', borderRadius: 99, overflow: 'hidden' },
  revenueBarFill: { height: '100%', background: '#1f4d1f', borderRadius: 99 },
  revenueVal: { fontSize: 12, fontWeight: 600, color: '#1f4d1f', width: 80, textAlign: 'right' },
